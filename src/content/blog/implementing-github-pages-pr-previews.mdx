---
title: "Implementing PR Previews with GitHub Pages and Astro"
description: "A complete guide to setting up PR preview deployments for Astro sites using GitHub Pages and Actions, without managing a gh-pages branch."
pubDate: November 28 2025
---

> Note: This blog post was written by Claude Code, an AI coding assistant. The implementation was completed through a conversation with Joe, where Claude Code both planned and executed the solution.

If you've ever worked on a site with multiple contributors, you know how valuable preview deployments are. Being able to see exactly how a change looks before merging to production catches bugs early and makes collaboration much smoother.

This post walks through implementing PR preview deployments for an Astro static site using GitHub Pages and GitHub Actions, without needing to manage a `gh-pages` branch or accumulate artifacts over time.
...
**Gotcha:** In Astro components, all `import` statements must be at the top of the frontmatter section, before any code execution. Don't put imports after variable declarations.
...
**Build fails with "Unterminated string literal"**

-   Check for syntax errors in template string interpolation
-   Make sure optional chaining (`?.`) isn't used directly in string concatenation - extract to a variable first

**Styles don't load in previews**

-   Verify `BASE_PATH` is being set correctly
-   Check that asset URLs use `getUrl()` or Astro's built-in asset handling
-   Look in browser console for 404s on CSS/JS files

**Active link highlighting broken**

-   Make sure link detection logic accounts for the base path
-   Strip base path from current pathname before comparing

**Different dependencies between PRs cause issues**

-   Ensure `npm ci` runs for each PR in the loop (line 74 in the workflow above)
...
-   [This site's implementation](https://github.com/JoeDuncko/astro-joeduncko.com)
